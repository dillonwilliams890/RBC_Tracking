...
def mass(b,pt):
    x=pt[0]; y=pt[1]
    parea=6.9
    Hb=(b[int(y-20):int(y+20), int(x-20):int(x+20)])
    base=Hb[20,20]*1.5
...
def masking(img, pt):
    x=pt[0]; y=pt[1]  
    img=(img[int(y-50):int(y+50), int(x-50):int(x+50)])
    ...
    for cnt in contours:
            ...
            if area > 500 and area < 4000:
                ...
                img=rescale(img)
                cell=(img[int(cy-40):int(cy+41), int(cx-40):int(cx+41)])
                cell_mask=(cell_mask[int(cy-40):int(cy+41), int(cx-40):int(cx+41)])
                masked=rescale_blue(cell,cell_mask)
    if masked.shape!=(81,81) or cell_mask.shape!=(81,81):
            masked=np.zeros((81,81))
            cell_mask=np.zeros((81,81))
...
def volume(img, pt, mask):
    channel=8
    x=pt[0]; y=pt[1]
    cell=(img[int(y-40):int(y+41), int(x-40):int(x+41)])
    if len(mask[0])<81:
        vol=np.nan
    else:
        kernel = np.ones((5,5),np.uint8)
        ...
        masked = (cell*(1-dilation))
        masked = (masked*(dilation_outer))
        sur=np.mean(masked)
...
def saturation(frame, pt430, pt410): #Calcuate cell saturation
    ...
    mass430=mass(b[0], pt430)
    mass410=mass(b[1], pt410)
...
def net(frame, pt430, pt410):
    ...
    imgR430=(R430[int(ptres430[1]-40):int(ptres430[1]+41), int(ptres430[0]-40):int(ptres430[0]+41)])
    imgR410=(R410[int(ptres410[1]-40):int(ptres410[1]+41), int(ptres410[0]-40):int(ptres410[0]+41)])
    mask430[mask430<0] = 0
    mask410[mask410<0] = 0
    imgB410[imgB410<0] = 0
    imgB430[imgB430<0] = 0
...
def segment(frames,x_old, y_old,BL):
    ...
    pt430=euler(f430,thresh, x_old, y_old)
    pt410=euler(f410,thresh, x_old, y_old)
...
def main_run(video): #Run the anaylsis in the video
    ...
    while i<len(video):
        ...
        # print(pt430)
        # print(pt410)
        if pt430[0]>26 and pt410[0]>26 and pt430[0]<334 and pt410[0]<334 and pt430[1]>26 and pt410[1]>26 and pt430[1]<234 and pt410[1]<234:
            sats=saturation(frame,pt430,pt410)
            ...
            x.append(0)
        i=i+1
    return imgs, saturations, volumes, x
...
def veiw(video):
    ...
    while i<len(video):
        ...
        if pt430[0]>26 and pt410[0]>26 and pt430[0]<334 and pt410[0]<334 and pt430[1]>26 and pt410[1]>26 and pt430[1]<234 and pt410[1]<234:
            ...
            pt410= np.asarray(pt410, dtype=np.float32)
            ptres430=2*pt430; ptres410=2*pt410
            x=int(ptres430[0]); y=int(ptres430[1])
            cv.rectangle(gray, (x-20, y-20), (x+20, y+20), (255,100,200),2)
            cv.putText(gray, str('%.2f' %sat), (x, y + 45), cv.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 2)
